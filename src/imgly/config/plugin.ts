...
export class VideoEditorConfig implements EditorPlugin {
  ...
  async initialize({ cesdk, engine }: EditorPluginContext) {
    if (cesdk) {
      ...
      // - videoDecode: 'block' - Prevents initialization if browser cannot decode video
      // - videoEncode: 'warn' - Shows warning if browser cannot encode video but allows editing
